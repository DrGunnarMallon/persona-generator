...
def load_interviews_from_mongo(uri):
    """
    Connects to MongoDB, fetches interviews along with associated self data, and returns them.

    Args:
        uri (str): MongoDB connection URI.

    Returns:
        List[dict]: A list of dictionaries where each dictionary contains self and their associated interviews.
    """
    try:
        # Connect to MongoDB
        ...
        # Fetch interviews and associated self data
        ...
        for interview in interviews_cursor:
            ...
            try:
                ...
                print(f"Error fetching self with ID {persona_id}: {e}")
                persona = None

            # Append interview and associated self data
...
def format_persona(self):
    """
    Formats self details into a readable string format.

    Args:
        persona (dict): Dictionary containing self details.

    Returns:
        str: Formatted string with self details.
    """
    details = []
...
def format_json_to_text(json_data):
    ...
    Args:
        json_data (dict): The JSON data containing self and interview.

    Returns:
        str: Formatted string with self details and interview Q&A.
    """
...
def save_interviews_to_files():
    ...
    for interview in interviews_data:
        output_file = (f"{interview['self']['first_name']}_{interview['self']['last_name']}_interview.txt")
        ...
        save_to_text_file(f"interviews/{output_file}", formatted_content)
